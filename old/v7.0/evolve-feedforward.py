...
def run():
    if True:
        population = neat.Checkpointer.restore_checkpoint('ckpt/ckpt-97563')
        data.total_wins = 92
        data.total_draws = 71
        data.total_losses = 737516602

        data.extinctions = 634
        ...
        with open("terminal_output", 'w') as to:
            to.write("")

    data.stats = neat.StatisticsReporter()
    population.add_reporter(data.stats)
    population.add_reporter(neat.StdOutReporter(True))
    population.add_reporter(neat.Checkpointer(
        20, filename_prefix='ckpt/ckpt-'))
    population.add_reporter(TeamReplacer(True))
